...
ElizaBot.prototype.reset = function() {
	...
	this.mem=[];
	this.lastchoice=[];
	for (var k=0; k<ElizaBot.elizaKeywords.length; k++) {
		this.lastchoice[k]=[];
		var rules=ElizaBot.elizaKeywords[k][2];
...
ElizaBot.prototype._init = function() {
	...
	var synPatterns={};
	if ((ElizaBot.elizaSynons) && (typeof ElizaBot.elizaSynons == 'object')) {
		for (var i in ElizaBot.elizaSynons) synPatterns[i]='('+i+'|'+ElizaBot.elizaSynons[i].join('|')+')';
	}
	// check for keywords or install empty structure to prevent any errors
	if ((!ElizaBot.elizaKeywords) || (typeof ElizaBot.elizaKeywords.length == 'undefined')) {
		ElizaBot.elizaKeywords=[['###',0,[['###',[]]]]];
	}
	...
	for (var k=0; k<ElizaBot.elizaKeywords.length; k++) {
		var rules=ElizaBot.elizaKeywords[k][2];
		ElizaBot.elizaKeywords[k][3]=k; // save original index for sorting
	...
	ElizaBot.elizaKeywords.sort(this._sortKeywords);
	// and compose regexps and refs for pres and posts
	ElizaBot.prototype.pres={};
	ElizaBot.prototype.posts={};
	if ((ElizaBot.elizaPres) && (ElizaBot.elizaPres.length)) {
		var a=new Array();
		for (var i=0; i<ElizaBot.elizaPres.length; i+=2) {
			a.push(ElizaBot.elizaPres[i]);
			ElizaBot.prototype.pres[ElizaBot.elizaPres[i]]=ElizaBot.elizaPres[i+1];
		}
	...
	if ((ElizaBot.elizaPosts) && (ElizaBot.elizaPosts.length)) {
		var a=new Array();
		for (var i=0; i < ElizaBot.elizaPosts.length; i+=2) {
			a.push(ElizaBot.elizaPosts[i]);
			ElizaBot.prototype.posts[ElizaBot.elizaPosts[i]] = ElizaBot.elizaPosts[i+1];
		}
	...
	if ((!ElizaBot.elizaQuits) || (typeof ElizaBot.elizaQuits.length == 'undefined')) {
		ElizaBot.elizaQuits=[];
	}
	// done
...
ElizaBot.prototype.transform = function(text) {
	...
	for (var i=0; i<parts.length; i++) {
		...
		if (part!='') {
			...
			for (var q=0; q<ElizaBot.elizaQuits.length; q++) {
				if (ElizaBot.elizaQuits[q]==part) {
					this.quit=true;
			...
			console.log(ElizaBot.elizaKeywords.length);
			for (var k=0; k<ElizaBot.elizaKeywords.length; k++) {
				if (part.search(new RegExp('\\b'+ElizaBot.elizaKeywords[k][0]+'\\b', 'i'))>=0) {
...
ElizaBot.prototype._execRule = function(k) {
	var rule=ElizaBot.elizaKeywords[k];
	var decomps=rule[2];
	var paramre=/\(([0-9]+)\)/;
	for (var i=0; i<decomps.length; i++) {
		...
		if (m!=null) {
			...
			var rpl=reasmbs[ri];
			if (this.debug) alert('match:\nkey: '+ElizaBot.elizaKeywords[k][0]+
				'\nrank: '+ElizaBot.elizaKeywords[k][1]+
				'\ndecomp: '+decomps[i][0]+
				'\nreasmb: '+rpl+
				'\nmemflag: '+memflag);
...
ElizaBot.prototype._getRuleIndexByKey = function(key) {
	for (var k=0; k<ElizaBot.elizaKeywords.length; k++) {
		if (ElizaBot.elizaKeywords[k][0]==key) return k;
	}
	return -1;
}
...
ElizaBot.prototype.getFinal = function() {
	if (!ElizaBot.elizaFinals) return '';
	return ElizaBot.elizaFinals[Math.floor(Math.random()*ElizaBot.elizaFinals.length)];
}

ElizaBot.prototype.getInitial = function() {
	if (!ElizaBot.elizaInitials) return '';
	return ElizaBot.elizaInitials[Math.floor(Math.random()*ElizaBot.elizaInitials.length)];
}
